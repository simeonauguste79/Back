...
$showNag = get_site_option('formcraft_basic_nag');
// $showNag = false;

if ($showNag == false) {
	update_site_option('formcraft_basic_nag', 'enabled');
	update_site_option('formcraft_basic_nag_last', strtotime('-3days'));
} else if ($showNag == 'disabled') {

} else if ($showNag == 'enabled' && ((strtotime('now') - get_site_option('formcraft_basic_nag_last')) / 86400) > 7) {
	update_site_option('formcraft_basic_nag_last', strtotime('now'));
	$showNagNow = true;
}
...
if ($showNagNow) {
	...
	<script>
		jQuery(document).ready(function() {
			setTimeout(function() {
				jQuery('#rate_us_modal').fcbmodal('show');
			}, 2000)
		})
	</script>
	<?php
...
<div class='formcraftbasic-css'>
	...
	<div class='row'>
						...
						if ( $total_forms>0 )
						{
							foreach ($forms as $key => $value) {
								...
								<div class='tr form-<?php echo $value->id; ?>'>
									...
									<span style='width:10%'><i data-id='<?php echo $value->id; ?>' class='fcb-icon trash-form'>delete</i></span>
								</div>
								<?php
							}
						}
		...
		<div class='large-6 column subs_cover'>
			...
			<h2><?php _e('Your Submissions','formcraft_basic'); ?> <span id='total-submissions'></span></h2>
			<div class='block'>
				<div class='table_list subs_list <?php echo $total_subs==0?'no-subs':''; ?>' cellpadding='0' cellspacing='0'>
					...
					<div class='no-subs-content'><?php _e('No Submissions','formcraft_basic'); ?>
					</div>
				</div>
			</div>
		</div>	
	</div>
<div class="fcbmodal fcbfade" id="rate_us_modal">
	<div class="fcbmodal-dialog" style="width: 500px">
		<div class="fcbmodal-content">
			<div class='fcbmodal-body'>
				<p>
					I am so sorry for the nag
				</p>
				<p>
					Would you like to take a minute to rate FormCraft?
				</p>
				<br>
				<a onClick="nagUpdate('disable')" href='https://wordpress.org/support/view/plugin-reviews/formcraft-form-builder#new-post' target='_blank' style='color: #077307'>
					Yes, if it's quick (opens review page in a new window).
				</a>
				<a onClick="nagUpdate('postpone')" style='color: #d42020'>
					Not now. Busy. Remind me in a week though.
				</a>
				<a onClick="nagUpdate('disable')" style='color: #d42020'>
				...
				<p>
					Your Review = More Downloads.<br>
					More downloads + New Reviews = We Get Stoked.<br>
					We Get Stoked = We Add New Features.<br>
				</p>
				<p>
					Win-Win!
				</p>
...
<div class="fcbmodal fcbfade" id="upgrade_modal">
	<div class="fcbmodal-dialog" style="width: 540px">
		<div class="fcbmodal-content">
			...
			<div class='fcbmodal-footer'>
				<a class='button blue' target='_blank' href='http://codecanyon.net/item/formcraft-premium-wordpress-form-builder/5335056?ref=ncrafts&source=basic'><?php _e('Get FormCraft Premium','formcraft_basic'); ?></a>
			</div>
		</div>
	</div>
</div>
