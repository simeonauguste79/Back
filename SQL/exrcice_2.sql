...
SELECT num_chambre chambres, nb_pers FROM chambres;
...
num_chambre, num_client FROM reservation WHERE date_arrive = '2019-03-15';

--6/Les numéros des chambres coûtant au minimum 80 Euro ayant un bain et volant au maximum 120 Euro
SELECT num_chambre FROM chambres WHERE nb_lit = '2' AND prix BETWEEN 80 AND 120;
...
SELECT COUNT(id_chambre) FROM chambres WHERE prix BETWEEN 200 AND 400;
...
--10/Jointure entre les tables client et réservation avec AS(alias) et INNER JOIN
SELECT*FROM clients AS c INNER JOIN,   AS r WHERE c.num_client = r.num_client;
----------------------------------------------------JOINTURE-------------------------------------------------------------
